...
with open(DATA_DIR / "unit.toml", "rb") as f:
    Slist = tomllib.load(f)["Slist"]
with open(DATA_DIR / "change.toml", "rb") as f:
    change = tomllib.load(f)
    Clist = change["Clist"]
    CClist = change["CClist"]
    CCClist = change["CCClist"]
with open(DATA_DIR / "build.toml", "rb") as f:
    Ilist = tomllib.load(f)["Ilist"]
with open(DATA_DIR / "upgrade.toml", "rb") as f:
    upgrade_times = tomllib.load(f)["Ulist"]
...
def collect_upgrade_events(event_categories, replay, upgrade_times):
    """
    7.统计UpgradeCompleteEvent事件，记录科技升级
    """
    Upgrade_events=event_categories['UpgradeCompleteEvent']
    ...
    for event in Upgrade_events:
        ...
        if event.second!=current_second: 
            while current_second!=event.second:
                current_second+=1
                for j in ue_players:
                    ue_players[j][current_second]=[]
        if event.upgrade_type_name in upgrade_times and current_second!=0 and event.player:
            try:
                ue_players[event.player][event.second-round(upgrade_times[event.upgrade_type_name][1]*1.4)].append(upgrade_times[event.upgrade_type_name][0])
            except:
                ue_players[event.player][event.second].append(upgrade_times[event.upgrade_type_name][0]+"（升级完毕）")
        #else:
        #    ue_players[event.player][event.second].append(event.upgrade_type_name)
    return ue_players

def Analysis(replaypath,rep_basedir):
    """
    Parameters:
    ----------
    replaypath: replay文件路径
    rep_basedir: replay文件所在目录

    Variables:
    ----------
    rep_name: replay文件名
    replay: sc2reader解析后的replay对象
    Duration: 游戏总持续时间(秒)
    output_path: 分析输出路径：./SC2Repanalysis/"replay文件名"
    event_categories: 存储分类后的事件字典
    current_second: 当前游戏内时间(秒)

    """
    try:
        rep_name = os.path.basename(replaypath)
        replay=sc2reader.load_replay(replaypath,load_level=4)
        Duration=round((replay.frames)/16)
        output_path="{}\\SC2RepAnalysis\\{}".format(rep_basedir,rep_name)

        LOG.info("分析输出路径: %s", output_path)
        ...
        event_categories=get_event(replay)        # 按类对Events进行分类
        ...
        unit_born_events = event_categories["UnitBornEvent"]
            ...
            if i.unit.owner and i.unit.name!='InvisibleTargetDummy'and i.unit.name!='ParasiticBombRelayDummy' and i.unit.name!='ParasiticBombDummy'and i.unit.name!='BroodlingEscort' and i.unit.name!='Broodling' and i.unit.name!='Larva':
                ...
                #考虑特殊的情况：跳秒（该秒没有发生建造事件）
                    ...
                    for j in ube_players:
                        ube_players[j][current_second]=[]

                #处理本秒内的情况
                if i.unit.name in Slist and current_second!=0:
                    try:
        ...
        #------------------------------------4.批量统计：单位出生与死亡---------------------------#
        RelatedEvent=[event_categories['UnitBornEvent']]
        if 'UnitDiedEvent' in event_categories:
            RelatedEvent.append(event_categories['UnitDiedEvent'])
        if 'UnitInitEvent' in event_categories:
            RelatedEvent.append(event_categories['UnitInitEvent'])
        if 'UnitTypeChangeEvent' in event_categories:
            RelatedEvent.append(event_categories['UnitTypeChangeEvent'])
        Fullinf=[]
        ...
        ue_players = collect_upgrade_events(event_categories, replay, upgrade_times)

        
        #--------------------------8.统计建筑建造列表----------------------------#
        if 'UnitInitEvent' in event_categories:
            unit_init_events = event_categories['UnitInitEvent']
                    ...
                    if i.second!=current_second: 
                        while current_second!=i.second:
                            ...
                            for j in uie_players:
                                uie_players[j][current_second]=[]
                    if i.unit_type_name in Ilist and current_second!=0:
        ...
        if 'UnitTypeChangeEvent' in event_categories:
            unit_change_events = event_categories['UnitTypeChangeEvent']
                    ...
                    if i.second!=current_second: 
                        while current_second!=i.second:
                            ...
                            for j in uce_players:
                                uce_players[j][current_second]=[]
                    if i.unit_type_name in Clist and current_second!=0:
                        #如果是虫族升本，就归类到科技升级上去
        ...
        player_stats_event=event_categories['PlayerStatsEvent']
        ...
        #-------------------------------11.格式化输出------------------------------------------#
        file = open('{}/{}.txt'.format(output_path,rep_name), 'w',encoding="utf-8")
        DataForPd={}
        PdS=[]
        ...
        for i in DataForPd:
            ...
            Inf_to_excel.to_excel("{}/{}的建造列表({}).xlsx".format(output_path,i,rep_name[:-10]),index=False)
        
        return 0
        # return PdS
    except Exception as e:
        LOG.info("Analysis error：%s",e)
